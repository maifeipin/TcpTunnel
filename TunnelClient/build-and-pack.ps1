...
Write-Host "  PUBLIC_PORT  = $PublicPort"
...
# ══════════════════════════════════════════════════════════════
#  4. 生成 tunnel-client.json
# ══════════════════════════════════════════════════════════════
$jsonPath = Join-Path $OutDir "tunnel-client.json"
$jsonContent = @"
{
    "server": {
        "host": "$ServerIp",
...
Write-Info "tunnel-client.json 已生成 → $jsonPath"

# ══════════════════════════════════════════════════════════════
#  5. 生成 start.bat（基于 --config，双击即可运行）
# ══════════════════════════════════════════════════════════════
...
echo [TunnelClient] 正在连接 %SERVER_IP% ...
echo 关闭此窗口即断开 RDP 隧道
echo.

TunnelClient.exe --config tunnel-client.json
...
# ══════════════════════════════════════════════════════════════
#  6. 生成快捷 start.ps1（PowerShell 版，支持 UTF-8 中文）
# ══════════════════════════════════════════════════════════════
$ps1Path = Join-Path $OutDir "start.ps1"
$ps1Content = @"
# TunnelClient 启动脚本 (PowerShell 版)
# 运行: powershell -ExecutionPolicy Bypass -File start.ps1
Write-Host "[TunnelClient] 读取配置 tunnel-client.json" -ForegroundColor Green
& "`$PSScriptRoot\TunnelClient.exe" --config "`$PSScriptRoot\tunnel-client.json"
"@
...
Write-Host "    3. 在任意位置用 mstsc 连接: $ServerIp`:$PublicPort"
Write-Host "══════════════════════════════════════════════════" -ForegroundColor Green

# ══════════════════════════════════════════════════════════════
#  8. 配置本机 Windows 防火墙（开放 RDP 入站端口）
